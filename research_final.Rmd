...
# Performance metrics - handle case where not all classes are predicted
# Create a full 2x2 matrix to handle missing predictions
full_matrix <- matrix(0, nrow = 2, ncol = 2,
                      dimnames = list(Actual = c("0", "1"), Predicted = c("0", "1")))

# Fill in values from conf_matrix
for (i in rownames(conf_matrix)) {
  for (j in colnames(conf_matrix)) {
    full_matrix[i, j] <- conf_matrix[i, j]
  }
}
...
recall <- ifelse((TP + FN) > 0, TP / (TP + FN), 0)  # Sensitivity
...
cat("\nNote: With imbalanced data, the default threshold of 0.5 may not be optimal.\n")
cat("Consider using a lower threshold to improve recall for fraud detection.\n")
```
