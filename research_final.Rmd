...
data <- data %>%
  mutate(
    ...
    # US location indicator (for salary analysis - avoid currency issues)
...
# Filter to only US entries with valid salary data (avoid currency issues)
...
```{r fig.width=10, fig.height=5}
# Calculate fraud probability by salary category (US only to avoid currency issues)
...
```{r}
cat("=== Salary Statistics (US Only) ===\n\n")

# Filter for US only to avoid currency issues
...
cat("2. SALARY PATTERNS (US only to avoid currency issues):\n")
cat("   - Real postings mean salary:", round(mean(real_data_us$salary_mid, na.rm = TRUE), 0), "\n")
cat("   - Fake postings mean salary:", round(mean(fraudulent_data_us$salary_mid, na.rm = TRUE), 0), "\n")
...
# For salary: only use US postings to avoid currency issues
...
## 7.1 Hypothesis 1: Mean Salary Comparison (US Only)
...
```{r}
# Create a clean dataset for logistic regression (US only to avoid currency issues)
model_data <- data %>%
  filter(is_us) %>%
  mutate(
    # Standardize salary (use midpoint, fill NA with median from US data)
...
# Print numeric columns rounded, keep Significant as is
...
### 9.9.2 Performance at Optimal Threshold
...
# Classification at default threshold 0.5 for comparison
...
cat(sprintf("%-20s %10s %10s\n", "Metric", "Optimal", "Default"))
cat(sprintf("%-20s %10s %10s\n", paste(rep("-", 20), collapse=""),
            paste(rep("-", 10), collapse=""), paste(rep("-", 10), collapse="")))
cat(sprintf("%-20s %9.2f%% %9.2f%%\n", "Accuracy",
            metrics_optimal$accuracy * 100, metrics_default$accuracy * 100))
cat(sprintf("%-20s %9.2f%% %9.2f%%\n", "Precision",
            metrics_optimal$precision * 100, metrics_default$precision * 100))
cat(sprintf("%-20s %9.2f%% %9.2f%%\n", "Recall",
            metrics_optimal$recall * 100, metrics_default$recall * 100))
cat(sprintf("%-20s %9.2f%% %9.2f%%\n", "Specificity",
            metrics_optimal$specificity * 100, metrics_default$specificity * 100))
cat(sprintf("%-20s %10.4f %10.4f\n", "F1 Score",
            metrics_optimal$f1, metrics_default$f1))

cat("\nInterpretation:\n")
cat("- Optimal threshold balances precision and recall for fraud detection\n")
cat("- Lower threshold increases recall (catches more frauds) at cost of some precision\n")
cat("- For fraud detection, higher recall is often preferred to minimize false negatives\n")
...
# Find the point on ROC curve for optimal threshold
...
cat("\nPREDICTION PERFORMANCE (at optimal threshold =", round(optimal_threshold, 3), "):\n")
